...
function filterFundedOnly() {
    ...
    console.log(fundedGames);    // use the function we previously created to add funded games to the DOM
...
const unfundedCount = GAMES_JSON.reduce((accumulator, game) => {
    return game.pledged < game.goal ? accumulator + 1 : accumulator;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const descriptionString = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} ${totalGames === 1 ? 'game' : 'games'}. Currently, ${unfundedCount} ${unfundedCount === 1 ? 'game remains' : 'games remain'} unfunded. We need your help to fund these amazing games!`;
